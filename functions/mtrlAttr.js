...
const getWithMtrlAttr = async (url, tbl_name, conflict_column) => {
  const allData = await FetchData(url);
  const lengthOfData = allData.length;

  for(let i=0; i<lengthOfData; i++){
    allData[i].mtrl_attr_unit_row_id = await getMtrlAttrUnitRowId(allData[i].mtrl_guid, allData[i].attr_guid);
    delete allData[i].mtrl_guid;
    delete allData[i].attr_guid;
  }

  let columnArray = Object.keys(allData[0]); //update columnArray
...
module.exports = getWithMtrlAttr;